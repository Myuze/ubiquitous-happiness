...
const { Game , Post } = require('../models');
...
router.get('/', withAuth, async (req, res) => {
  ...
  if (user){
    ...
    const games = gameData.map((fPost) => fPost.get({ plain: true }));
    res
      .status(200)
      .render('home', { games });
  } else {
    const gameData = await Post.findAll()
      .catch((err) => {
        res.json(err);
      });
    const forumPosts = gameData.map((fPost) => fPost.get({ plain: true }));
